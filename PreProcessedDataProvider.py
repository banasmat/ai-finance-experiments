...
class PreProcessedDataProvider(object):
    ...
    def get_price_data(self, refresh=True):

        if refresh is not True:
            return pd.DataFrame.from_csv('output/pre_prices.csv')
        ...
        prices.to_csv('output/pre_prices.csv')

        return prices

    def get_news_data(self, from_datetime, refresh=True):

        if refresh is not True:
            return pd.DataFrame.from_csv('output/pre_news.csv')
        ...
        news.to_csv('output/pre_news.csv')
